...
contract Cfn is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the CFN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("CaffeineFund", "CFN") {
        // Mints 1 CFN to contract creator for initial pool setup
    ...
    function _getCfnPrice() internal view returns (uint256 _cfnPrice) {
        try IOracle(cfnOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Cfn: failed to fetch CFN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _cfnPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cfnPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCfnOracle(address _cfnOracle) public onlyOperatorOrTaxOffice {
        require(_cfnOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints CFN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of CFN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCfnPrice = _getCfnPrice();
            currentTaxRate = _updateTaxRate(currentCfnPrice);
            if (currentCfnPrice < burnThreshold) {
                burnTax = true;
            }
        }
